# Функція для зчитування матриці з вхідних даних
def read_matrix():
...
# Функція для транспонування матриці за вертикаллю
def transpose_vertical_line(matrix, rows, cols):
    transposed_matrix = [row[::-1] for row in matrix]
    return transposed_matrix


# Функція для транспонування матриці за горизонталлю
...
# Функція для отримання допоміжної матриці (мінора)
